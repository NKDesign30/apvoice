...
const gaeMapper = (function () {
  ...
  if (typeof ga_options !== 'undefined') {
    ...
    // from DB section
    const clickElementsFromDB = [];

    const click_elements_from_options = ga_options.click_elements;
    for (let i = 0; i < click_elements_from_options.length; i++) {
      const clicked = click_elements_from_options[i];
      newClickElement = {};
    ...
    const scroll_elements_from_options = ga_options.scroll_elements;
    const scrollElementsFromDB = [];

    // NB these will be assigned meaningful names in backend eventually
    if (typeof scroll_elements_from_options !== 'undefined') {
      scroll_elements_from_options.forEach(el => {
...
gaEventsMain = (function ($) {
  ...
  function applyBindings() {
    ...
    gaeMapper.clickElementsFromDB.forEach(el => {
  ...
  function bindScrollEventsFromDB() {
    ...
    const ga_window = $(window).height();
    const ga_scroll_top = $(document).scrollTop();

    for (let i = 0; i < gaeMapper.scrollElementsFromDB.length; i++) {
      if (!gaeMapper.scrollElementsFromDB[i].sent) {
        // NB was unescapeChars( gaeMapper.scrollElementsFromDB[i].select)
        const $select = $(gaeMapper.scrollElementsFromDB[i].select);
  ...
  function bindDownloadTrackingEvents() {
    if (gaeMapper.downloadTracking === '1') {
      gaeMapper.downloadTrackingFileTypes.forEach(ftype => {
        const lowercase = ftype.toLowerCase();
        const uppercase = ftype.toUpperCase();
  ...
  function bindEmailLinksTracking() {
    if (gaeMapper.emailLinksTracking === '1') {
      $('body').on('click', 'a[href^="mailto:"]', function (e) {
        e.preventDefault();
        const label = this.href.split(':').pop();
        const action = 'Email Link';
        const category = getPageName();
  ...
  function bindTelLinksTracking() {
    if (gaeMapper.telLinksTracking === '1') {
      $('body').on('click', 'a[href^="tel:"]', function (e) {
        e.preventDefault();
        const label = this.href.split(':').pop();
        const action = 'Telephone Number Link';
        const category = getPageName();
        track_event(category, action, label, false, '');
        handleLinks(this, e);
      });
  ...
  var track_event = function (category, action, label, bounce, evalue) {
    ...
    const event_category = !category ? 'uncategorized' : category;
    category = event_category;

    const event_action = !action ? '' : action;
    action = event_action;

    const event_label = !label ? '' : label;
    label = event_label;

    const event_value = !evalue ? '' : evalue;
    const event_bounce = !bounce ? false : bounce;
  ...
  var handleLinks = function (self, event) {
    ...
    const link = getUrl(event);


    let w;
    const openInNewTab = isNewTab(self);
    if (openInNewTab) {
      w = window.open('', '_blank');
    }

    const hash = isJustHashLink(link);
