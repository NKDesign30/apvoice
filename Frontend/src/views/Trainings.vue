      ...
      >
        ...
        >
          <div
            ...
            style="background: linear-gradient(to right,#069BFF,#069BFF,#069BFF, #ABDDEF, #ABDDEF);"
            @click="handleCategoryClick"
          >
            <span class="flex-grow flex-shrink text-center text-white">{{ $t("trainings.categoryTrainings.headline") }}</span>
            <img
          ...
          >
            ...
            <!-- Hinzugefügter Kategoriefilter -->
            <div
              v-if="productCategory !== null && categoryCategory !== null && productCategory !== undefined && categoryCategory !== undefined"
            ...
            >
              ...
              >
                <div v-if="category.id !== productCategory.id && category.id !== categoryCategory.id">
                  <apo-icon
...
export default {
  ...
  computed: {
    ...
    trainingCategoriesForFilter() {
      // there must be trainings of this category available for both product and category training series
      const availableProductCategoryIds = this.productTrainingSeries
      ...
      const availableCategoryCategoryIds = this.categoryTrainingSeries
        .reduce((availableCategories, series) => [...availableCategories, ...series.categories], [])
        .filter(id => id !== this.categoryCategory.id && id !== this.productCategory);

      const availableCategoryIds = [...new Set([...availableProductCategoryIds, ...availableCategoryCategoryIds])];
