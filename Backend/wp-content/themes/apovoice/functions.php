...
{
    $url = 'https://backend.apovoice.es/wp-json/apovoice/v1/pharmacies-fuzzy-search/create-index';
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczpcL1wvYmFja2VuZC5hcG92b2ljZS5lcyIsImlhdCI6MTY5MDE5Mjc1MywibmJmIjoxNjkwMTkyNzUzLCJleHAiOjE2OTAxOTQ1NTMsImRhdGEiOnsidXNlciI6eyJwYXNzIjoiJFAkQk1LOXVWNlYzSzRnbklTN0ZRV3oxUXFtM3J5QzJRXC8iLCJpZCI6IjI5NDYxIn19fQ.8TcTVTMfqE969TiU9uxvM8DPAdvpw9a80Siv0gihQaw'
    ));
...
if (!wp_next_scheduled('run_indexing')) {
    wp_schedule_event(time(), 'weekly', 'run_indexing');
}

add_action('run_indexing', 'run_indexing');

//NL-nach 3 Monaten
add_action('rest_api_init', function () {
    register_rest_route('wc/v2', '/should_show_newsletter_popup/(?P<id>\d+)', array(
        'methods' => 'GET',
        'callback' => 'should_show_newsletter_popup'
    ));
    register_rest_route('wc/v2', '/update_last_newsletter_popup_date/(?P<id>\d+)', array(
        'methods' => 'POST',
        'callback' => 'update_last_newsletter_popup_date'
    ));
});

function update_last_newsletter_popup_date(WP_REST_Request $request)
{
    $user_id = $request['id'];
    $current_date = date('Y-m-d');
    update_user_meta($user_id, 'last_newsletter_popup_date', $current_date);
    return new WP_REST_Response(true, 200);
}

function get_last_newsletter_popup_date($user_id)
{
    return get_user_meta($user_id, 'last_newsletter_popup_date', true);
}

function should_show_newsletter_popup(WP_REST_Request $request)
{
    $user_id = $request['id'];
    $last_popup_date = get_last_newsletter_popup_date($user_id);
    if (!$last_popup_date) {
        return new WP_REST_Response(true, 200);
    }
    $current_date = date_create(date('Y-m-d'));
    $last_popup_date = date_create($last_popup_date);
    $interval = date_diff($current_date, $last_popup_date);
    return new WP_REST_Response($interval->format('%m') >= 3, 200);
}
