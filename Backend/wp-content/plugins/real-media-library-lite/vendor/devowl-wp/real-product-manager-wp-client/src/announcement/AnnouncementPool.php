...
class AnnouncementPool {
    ...
    private function __construct($pluginUpdate) {
        ...
        $this->view = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\view\AnnouncementView::instance(
            $this
        );
        $this->option = new \MatthiasWeb\RealMediaLibrary\Vendor\MatthiasWeb\Utils\ExpireOption(
            self::OPTION_NAME . '_' . $pluginUpdate->getInitiator()->getPluginSlug(),
            is_multisite(),
            self::OPTION_EXPIRE
        );
        $this->client = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\client\Announcement::instance(
            $pluginUpdate
        );
    }
    /**
    ...
    public function sync($force = \false) {
        ...
        if ($this->items === null) {
            ...
            foreach ($value['items'] as $row) {
                $this->items[] = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\announcement\Announcement::fromResponse(
                    $this,
                    $row
                );
            }
    ...
    public static function instance($pluginUpdate) {
        return new \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealProductManagerWpClient\announcement\AnnouncementPool(
            $pluginUpdate
        );
    }
}
