...
function edit_post($post_data = null)
{
	...
	if (isset($post_data['is_premium'])) {
		global $wpdb;

		if ($post_data['is_premium'] == "true") {
			$premium = 1;
		} else {
			$premium = 0;
		}

		// update_post_meta($post_ID, 'is_premium', $premium);
		$wpdb->query($wpdb->prepare("UPDATE " . $wpdb->get_blog_prefix(get_current_site_id()) . "posts SET is_premium=%s WHERE ID=%d", array($premium, $post_ID)));
	}
