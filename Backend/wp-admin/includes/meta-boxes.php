 ...
 * }
 */
function post_premium_content_meta_box($post, $box){
	//
	?>
	<div>
		<input <?php echo $post->is_premium == '1' ? 'checked' : '' ?> type="checkbox" id="is_premium_box">
		<label for="is_premium_box">Premium Content</label>
	</div>
	<script>
		let isPremium = document.getElementById('is_premium_box');
		const isPremiumInput = document.getElementById('is_premium');
		let premium = "";
		isPremium.addEventListener('change', function() {
			isPremiumInput.value = isPremium.checked;
			console.log(isPremiumInput.value);
		});
	</script>
	<?php
}
...
function register_and_do_post_meta_boxes( $post ) {
	 ...
	 */
	add_meta_box(
		'premium_content_meta_box',                 // Unique ID
		__( 'Premium Content' ),      // Box title
		'post_premium_content_meta_box',  // Content callback, must be of type callable
		$post_type,                            // Post type
		'side',
	);
